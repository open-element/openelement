/**
 * Playwright configuration for the app-flow-native fixture (#1339).
 *
 * Tests run against the built fixture app (packages/adapter-vite/__fixtures__/
 * app-flow-native/dist), served by e2e/server.ts: static files from disk,
...
const PORT = Number(process.env.APP_FLOW_NATIVE_E2E_PORT ?? 4280);
...
export default defineConfig({
  ...
  // The store is shared mutable server state: tests use relative assertions
  // (capture counts before, compare after) so all three browser projects can
  // share the one server in any order.
  fullyParallel: false,
