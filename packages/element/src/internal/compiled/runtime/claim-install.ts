 ...
 *
 * Imported for its side effect by `@openelement/element` (the default entry,
 * `packages/element/src/index.ts`) and by nothing else. Its only job is to
 * make the single claim implementation reachable from a graph that wants it:
 * dropping this one import is what turns the bundle into a client-only one
 * (see claim-seam.ts and packages/element/src/client-only.ts).
 *
