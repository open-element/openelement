...
源码包行为 `0.44.0-beta.2.2`（`v0.44.0-beta.2.2`）——当前公开 v0.44 预发布线
（Beta.2.2：Native/Lit 双渲染 Framework Mode 与 Document 语义收口，ADR-0152、ADR-0153）。
npm registry 行为 `v0.44.0-beta.2.2`——预发布版本(dist-tag `beta`)；npm `latest` 仍为
已发布的稳定 0.43 线。
...
是核心技术资产；每次替换同步清理旧实现。下一步为 Beta.2.3 收敛，验收后
