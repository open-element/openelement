...
| `@std` bridge stays in Deno-driven tooling only                                     | Packed `vite/` + `cli/` keep bare `@std/*` while the manifest declares `@jsr/std__*`; the public runtime graph is `@std`-free (verified per tarball) | `accepts the @std bridge only in Deno-driven tooling`; template + harness aliases | Pack rewrites tooling imports to the compat names, or the tooling drops `@std` |
