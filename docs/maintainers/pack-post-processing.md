...
| `findRawTypeScriptPayload` (fail-closed guard, deletes nothing)                                                                           | Raw pack retains unimported TypeScript sources; the marker modules that once required `publish.exclude` were removed (markers now live in imported protocol modules), so `publish.exclude` carries no source exclusions                                   | `findRawTypeScriptPayload flags sources but keeps declarations and templates`; raw `deno pack` of element/create shows zero non-declaration `.ts`                                                             | Delete the guard only when `deno pack` itself rejects unimported sources (until then a regression must fail the pack, never be silently stripped) |
...
| Package metadata fields (`type`, `description`, `repository`, `homepage`, `bugs`, `license`, `keywords`, `engines`, `sideEffects`, `bin`) | Raw `package.json` omits them                                                                                                                                                                                                                             | `package-artifacts:check` tarball scan; `pack-surface:check` compares the packed manifest against `packedMetadata()`; `assertOnlyApprovedManifestChanges` delta proof                                         | Pack propagates them from `deno.json`                                                                                                             |
...
collection, tarball safety checks (`package-artifacts:check`), the npm facade
check (`pack-surface:check`: metadata parity with `packedMetadata()`, no
repository-internal path or decision-record citation in shipped text, every
public subpath named in the shipped README), the declaration
