...
TypeScript is layered deliberately. Deno source is checked by Deno's own supported checker; the Element compiler and every other AST consumer use the classic TypeScript compiler API, reached only through the import-map name `typescript`, which maps to `npm:@typescript/typescript6@^6` so the API-less TypeScript 7 CLI package can never satisfy that import (the `@typescript/native-preview` and `tsgo` names are not used anywhere). The TypeScript 7 `tsc` CLI is exercised only as an independent shadow gate (`deno task gate:ts7:shadow`, non-blocking CI job `ts7-shadow`) over the packed Node/npm consumer matrix — typecheck, declaration emit, and build mode against a packed TS6 baseline — and it becomes a required gate only after a complete matching matrix.
