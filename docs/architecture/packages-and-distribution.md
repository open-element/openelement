...
TypeScript is layered deliberately. Deno source is checked by Deno's own supported checker; the Element compiler and every other AST consumer use the classic TypeScript compiler API, reached through the import-map name `typescript`, which maps directly to the exact `npm:typescript@6.0.3`. The TypeScript 7 `tsc` CLI remains an explicit shadow qualification (`deno task gate:ts7:shadow`) over the packed Node/npm consumer matrix; it is not part of every pull-request run while the disposable consumer's TS7 result is compared with the packed Element dependency's pinned TS6 baseline. It becomes a required gate only after a complete matching matrix.
