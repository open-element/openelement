...
以下数字于 2026-09-18 量自 docs 站点自身的构建（`www/dist`，在本仓库 commit `213d4fda` 上由 `deno task site:build` 生成）。下列命令可在该 commit 上复现每一行；页面数与 URL 数随路由集合变化，内容变更后请重跑。
...
| `island-open-cinematic-scroll` | 79,199   | 23,894  |
