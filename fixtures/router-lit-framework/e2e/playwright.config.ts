/**
 * Playwright configuration for the app-flow-lit fixture (Beta.2.2, #1339).
 *
 * Tests run against the built fixture app (fixtures/
 * app-flow-lit/dist), served by e2e/server.ts: static files from disk,
 * renderIntent 'dynamic' routes delegated to dist/server/index.js.
 *
 * Chromium, Firefox and WebKit — the same cross-browser acceptance matrix as
 * app-flow-native; the store is shared mutable server state, so tests use
 * relative assertions (capture counts before, compare after) and all three
...
const PORT = Number(process.env.APP_FLOW_LIT_E2E_PORT ?? 4281);
